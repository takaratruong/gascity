...
app.get('/exec/rig/activity', async (req, res) => {
  ...
  try {
    const [openResult, inProgressResult, proposalsResult, acceptedResult, terminalResult, sessionResult] = await Promise.all([
      runGC(['bd', 'list', '--status', 'open', '--limit', '1000', '--json']),
      runGC(['bd', 'list', '--status', 'in_progress', '--limit', '500', '--json']),
      runGC(['bd', 'list', '--label', 'kind:proposal', '--status', 'open', '--limit', '200', '--json']),
      runGC(['bd', 'list', '--label', 'status:accepted', '--all', '--limit', '100', '--json']),
      runGC(['bd', 'list', '--has-metadata-key', 'convergence.terminal_reason', '--all', '--limit', '100', '--json']),
      runGC(['session', 'list', '--json']),
    ]);

    const openItems: BeadLike[] = [
      ...(openResult.code === 0 ? parseJSONItems(openResult.stdout) : []),
      ...(inProgressResult.code === 0 ? parseJSONItems(inProgressResult.stdout) : []),
    ];
    const proposalItems: BeadLike[] = proposalsResult.code === 0 ? parseJSONItems(proposalsResult.stdout) : [];
    const resultItems = new Map<string, BeadLike>();
    for (const item of [
      ...(acceptedResult.code === 0 ? parseJSONItems(acceptedResult.stdout) : []),
      ...(terminalResult.code === 0 ? parseJSONItems(terminalResult.stdout) : []),
    ] as BeadLike[]) {
      if (item.id) resultItems.set(item.id, item);
    }
    const historicalItems = [...resultItems.values()];
    const sessions = sessionResult.code === 0 ? parseJSONItems(sessionResult.stdout) : [];

    const rigOpen = openItems.filter((b) => beadBelongsToRig(b, rig));
    const runCandidates = rigOpen
      .filter((b) => b.issue_type === 'convergence' || b.metadata?.['convergence.formula'])
      .sort((a, z) => String(z.updated_at ?? z.created_at ?? '').localeCompare(String(a.updated_at ?? a.created_at ?? '')));
    const runCandidateIds = new Set(runCandidates.map((b) => b.id).filter(Boolean));
    const candidateWorkBeads = rigOpen
      .filter((b) => b.issue_type !== 'convergence' && !(b.labels ?? []).includes('kind:proposal'))
      .filter((b) => runCandidateIds.has(b.metadata?.['gc.parent_run']) || runCandidates.some((run) => b.id?.startsWith(`${run.id}.`)));

    const runsWithOpenWork = new Set(candidateWorkBeads.map((b) => b.metadata?.['gc.parent_run']).filter(Boolean));
    const staleCutoffMs = 45 * 60 * 1000;
    const staleRuns = runCandidates.filter((b) => {
      const updated = String(b.updated_at ?? b.created_at ?? '');
      return !runsWithOpenWork.has(b.id) && ageMs(updated) > staleCutoffMs;
    });
    const activeRuns = runCandidates.filter((b) => !staleRuns.some((stale) => stale.id === b.id));
    const activeRunIds = new Set(activeRuns.map((b) => b.id).filter(Boolean));
    const workBeads = candidateWorkBeads
      .filter((b) => activeRunIds.has(b.metadata?.['gc.parent_run']) || activeRuns.some((run) => b.id?.startsWith(`${run.id}.`)))
      .sort((a, z) => String(z.updated_at ?? z.created_at ?? '').localeCompare(String(a.updated_at ?? a.created_at ?? '')));

    const proposals = proposalItems
      .filter((b) => (b.labels ?? []).includes('kind:proposal') && beadBelongsToRig(b, rig))
      .sort((a, z) => String(z.updated_at ?? z.created_at ?? '').localeCompare(String(a.updated_at ?? a.created_at ?? '')))
      .slice(0, 12);

    const acceptedRuns = historicalItems
      .filter((b) => beadBelongsToRig(b, rig))
      .filter((b) => b.issue_type === 'convergence' || b.metadata?.['convergence.formula'])
      .filter((b) =>
        b.metadata?.['gc.result_class'] === 'ACCEPTED_RESEARCH_SIGNAL' ||
        (b.labels ?? []).includes('status:accepted') ||
        b.metadata?.['convergence.terminal_reason'] === 'approved')
      .sort((a, z) => String(z.updated_at ?? z.closed_at ?? z.created_at ?? '').localeCompare(String(a.updated_at ?? a.closed_at ?? a.created_at ?? '')))
      .slice(0, 5);

    const latestResultRuns = historicalItems
      .filter((b) => beadBelongsToRig(b, rig))
      .filter((b) => b.issue_type === 'convergence' || b.metadata?.['convergence.formula'])
      .filter((b) => b.metadata?.['gc.result_class'] || b.metadata?.['convergence.terminal_reason'])
      .filter((b) => b.metadata?.['gc.result_class'] !== 'OPERATOR_REJECTED')
      .sort((a, z) => String(z.updated_at ?? z.closed_at ?? z.created_at ?? '').localeCompare(String(a.updated_at ?? a.closed_at ?? a.created_at ?? '')))
      .slice(0, 5);

    const heldProposals = proposals.filter((b) => (b.labels ?? []).some((label) => label.startsWith('status:held')));
    const openProposals = proposals.filter((b) => b.status === 'open');
    const activeDirectives = openItems
      .filter((b) => (b.labels ?? []).includes('kind:directive') && (b.labels ?? []).includes(`rig:${rig}`))
      .filter((b) => b.status === 'open' && (b.labels ?? []).includes('status:active'))
      .sort((a, z) => String(z.updated_at ?? z.created_at ?? '').localeCompare(String(a.updated_at ?? a.created_at ?? '')))
      .slice(0, 10);

    const relevantSessions = sessions
      .filter((s: any) => {
        const template = String(s.Template ?? s.template ?? '');
        const alias = String(s.Alias ?? s.alias ?? '');
        return template.includes(rig) || alias.includes(rig) || template === mayor || alias === mayor || template === 'curator-decider';
      })
      .map((s: any) => ({
        id: s.ID ?? s.id,
        session_name: s.SessionName ?? s.session_name,
        template: s.Template ?? s.template,
        alias: s.Alias ?? s.alias,
        state: s.State ?? s.state,
        last_active: s.LastActive ?? s.last_active,
        attached: s.Attached ?? s.attached,
      }));

    const activeSessionForRoute = (route: string): any | null =>
      relevantSessions.find((s: any) => {
        if (s.state !== 'active' && s.state !== 'creating') return false;
        return String(s.template ?? '') === route || String(s.alias ?? '').startsWith(`${route}-`);
      }) ?? null;

    const activeRunById = new Map(activeRuns.map((run) => [run.id, run]));
    const enrichedWorkBeads = workBeads.map((b) => {
      const route = String(b.metadata?.['gc.routed_to'] ?? '');
      const routeSession = route ? activeSessionForRoute(route) : null;
      const rootId = String(b.id ?? '').split('.')[0];
      const rootRun = activeRunById.get(rootId);
      const metadata = {
        ...(b.metadata ?? {}),
        ...(rootRun && !b.metadata?.['gc.parent_run'] ? { 'gc.parent_run': rootRun.id } : {}),
        ...(rootRun && !b.metadata?.['gc.thread_id'] ? { 'gc.thread_id': rootRun.metadata?.['gc.thread_id'] ?? rootRun.id } : {}),
        ...(rootRun && !b.metadata?.['gc.thread_title'] ? { 'gc.thread_title': rootRun.metadata?.['gc.thread_title'] ?? rootRun.title } : {}),
      };
      return {
        ...b,
        metadata,
        ui_worker_state: b.status === 'in_progress' ? 'claimed' : routeSession ? 'session-active' : route ? 'queued' : b.status,
        ui_worker_session: b.assignee || routeSession?.alias || routeSession?.session_name || null,
      };
    });

    const currentRun = activeRuns[0] ?? null;
    const currentStep = [...enrichedWorkBeads].sort((a, z) => {
      const rank = (b: BeadLike) => {
        const routedTo = String(b.metadata?.['gc.routed_to'] ?? '');
        const workerLeg = routedTo.includes('reviewer') || routedTo.includes('implementer');
        if (b.status === 'in_progress' && workerLeg) return 0;
        if ((b as any).ui_worker_state === 'session-active' && workerLeg) return 1;
        if (b.status === 'in_progress') return 2;
        if ((b as any).ui_worker_state === 'session-active') return 3;
        if (workerLeg) return 4;
        return 5;
      };
      return rank(a) - rank(z) || String(z.updated_at ?? z.created_at ?? '').localeCompare(String(a.updated_at ?? a.created_at ?? ''));
    })[0] ?? null;
    const lastAccepted = acceptedRuns[0] ?? null;
    const nextAction = currentRun
      ? `Running ${currentStep?.metadata?.['gc.routed_to'] || currentRun.metadata?.['convergence.target'] || 'worker'} on ${currentRun.id}`
      : openProposals.length > 0
        ? `${openProposals.length} proposal${openProposals.length === 1 ? '' : 's'} waiting for curator/mayor decision`
        : lastAccepted
          ? `Idle after accepted result ${lastAccepted.id}`
          : 'Idle; no accepted result yet';
    ...
    res.json({
      ...
      errors: {
        open: openResult.code === 0 ? null : openResult.stderr || openResult.stdout,
        in_progress: inProgressResult.code === 0 ? null : inProgressResult.stderr || inProgressResult.stdout,
        proposals: proposalsResult.code === 0 ? null : proposalsResult.stderr || proposalsResult.stdout,
        accepted: acceptedResult.code === 0 ? null : acceptedResult.stderr || acceptedResult.stdout,
        terminal: terminalResult.code === 0 ? null : terminalResult.stderr || terminalResult.stdout,
        sessions: sessionResult.code === 0 ? null : sessionResult.stderr || sessionResult.stdout,
      },
    });
  } catch (err: any) {
    res.status(500).json({ error: String(err) });
